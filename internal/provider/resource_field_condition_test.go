...
// fieldConditionBodyOnPhase is fieldConditionBody with the owning phase spelled
// out, so a mock can report a phase other than the one the write requested.
...
// fieldConditionPhaseState drives the mocks that reproduce the API attaching a
// condition to a phase other than the one the write asked for. Each operation
// reports its own phase, so a read can still find the condition where it was
// while an update relocates it.
...
// fieldConditionPhaseConfig always asks for phase_2, the phase the relocation
// mocks either honor or replace with the start form's phase_1.
...
// TestUnit_FieldConditionResource_CreatePhaseMismatchRollsBack covers a create
// on a phase the API does not honor: it attaches the condition to the start
// form instead and reports that phase back. Recording that phase fails the
// apply and taints the resource forever, so the provider deletes what it just
// created and says why the phase cannot be managed.
func TestUnit_FieldConditionResource_CreatePhaseMismatchRollsBack(t *testing.T) {
	...
	// The one delete is the rollback: a resource left in state would add a
	// second at destroy time, and one never created would leave none.
	if st.deleted != 1 {
		t.Fatalf("expected the rollback to delete the condition exactly once, got %d deletes", st.deleted)
	}
}

// TestUnit_FieldConditionResource_CreatePhaseMismatchRollbackFails covers the
// rollback itself failing: the condition exists on a phase Terraform is not
// tracking, and the error has to name both problems so someone can go delete it
// by hand.
...
// TestUnit_FieldConditionResource_UpdatePhaseMismatchFails covers an update
// whose response reports a different phase than the one under management. The
// condition already exists and prior state still describes it, so the update
// fails without a rollback and a later refresh reconciles the phase.
func TestUnit_FieldConditionResource_UpdatePhaseMismatchFails(t *testing.T) {
	...
	resource.UnitTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// The failed update must not have thrown the condition away: the API
			// cooperating again updates the same one instead of creating another.
			{
